...
const SignedOnTile: React.FC<SignedOnTileProps> = ({ room, trainers, onBreak, onSignOff, onUpdatePhoto, onEndBreak, onEditBreak }) => {
  ...
  const photoUrl = matchedTrainer?.photoUrl;
  ...
  return (
    ...
    >
      <button onClick={() => setExpanded(e => !e)} className="w-full flex items-center gap-3 p-3 text-left hover:bg-gray-50 transition-colors">
        <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-100 border-2 border-gray-100 shrink-0 flex items-center justify-center text-gray-500">
          {photoUrl && !photoFailed
            ? <img src={photoUrl} alt={room.trainer || 'Trainer'} onError={() => setPhotoFailed(true)} className="w-full h-full object-cover object-top" />
            : <span className="font-bold text-sm">{trainerInitials(room.trainer)}</span>}
        </div>
        ...
        {!srcDataUrl ? (
          <div className="p-6 space-y-4">
            <div className="flex flex-col items-center gap-3">
              <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-gray-100 bg-gray-50 flex items-center justify-center text-gray-500">
                {trainer.photoUrl
                  ? <img src={trainer.photoUrl} alt={trainer.name} className="w-full h-full object-cover object-top" />
                  : <span className="font-bold text-lg">{trainerInitials(trainer.name)}</span>}
              </div>
