...
const RoomItem: React.FC<{ room: RoomAllocation; trainers: Trainer[] }> = ({ room, trainers }) => {
  ...
  const trainerPhoto = matchedTrainer?.photoUrl;
    ...
    >
      <div className="flex flex-row items-center gap-3">
        ...
        {hasContent && room.trainer ? (
          <>
            <div className={`w-[3.5rem] h-[3.5rem] rounded-full overflow-hidden border-[3px] shrink-0 bg-white flex items-center justify-center ${
              isLive ? 'border-eqc-green/40 text-eqc-green' : isInactive ? 'border-gray-300 text-gray-500' : 'border-white/40 text-eqc-text'
            }`}>
              {trainerPhoto && !photoFailed ? (
                <img src={trainerPhoto} alt={room.trainer} onError={() => setPhotoFailed(true)} className="w-full h-full object-cover object-top" />
              ) : (
                <span className="font-sans font-bold text-base">{getInitials(room.trainer)}</span>
              )}
            </div>
